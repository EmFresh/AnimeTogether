...
namespace SimpleWebBrowser
{
    ...
    public class WebBrowser2D : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler,
        IPointerUpHandler
        {
            #region My Video Finding Stuff
            [HideInInspector]
            public string htmlString;
            public static string videoURL;
            private bool readHtml;

            int findStrIndexNS(string og, string find)
            {
                for (int i = 0; i < og.Length - find.Length; ++i)
                {
                    string tmp = og.Substring(i, find.Length);
                    if (tmp == find)
                        return i;
                }
                return -1;
